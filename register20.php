<?php
    ...
    if(isset($_POST["btnRegister"])){
        ...
        $picture = "profile.jpg";
        $acc = "SELECT * FROM user";
        $chck_result = mysqli_query($conn,$acc);
        $row = mysqli_fetch_assoc($chck_result);
        if($cfm_password === $rgt_password){
            if($username === "admin" || $username == $row['username']){

                echo "<script> alert('This username is used!!')</script>";
            }else{
                $sql = "INSERT INTO `user`(`username`, `password`, `fname`, `lname`, `dob`, `gender`, `email`, `icon`) 
                VALUES ('$username','$password','$fname','$lname','$dob','$gender','$email', '$picture')";
...
<head>
    ...
    <link rel="stylesheet" href="register30.css">
